...
- **業務報告シート (`/report`)**: 全案件 × 全土地 × 最新訪問記録を PDF版 業務報告シートと同じ列構成で一覧表示。
  `?projectId=...` で案件フィルタの初期値を指定できる
...
- 認証情報（`aid`/`kid`/`lmtinf`）は仕様上ファイル/DB へ永続化しない（プロセス内のみ）
...
| `GET /api/projects` | 全案件を lands・buildings・visits 込みのツリーで返す |
...
| `POST /api/projects/:id/lands/:landId/buildings` | 建物追加（所有形態 `sole`/`kubun`。owners は sole のみ・units は kubun のみ受け付け） |
| `PATCH /api/projects/:id/lands/:landId/buildings/:buildingId` | 建物の部分更新（owners / units は渡したときのみ全置換。所有形態変更で旧形態側の所有者情報を破棄） |
| `DELETE /api/projects/:id/lands/:landId/buildings/:buildingId` | 建物削除（専有部分・所有者もカスケード削除） |
| `POST /api/projects/:id/lands/:landId/visits` | 訪問記録追加（編集・削除 API は持たない） |
| `POST /api/reset` | 案件・土地・建物・訪問記録を破棄してサンプル再投入（筆マスタは残す） |
...
app/
  ...
  report/page.tsx             # 業務報告シート
  kouzu-map/page.tsx          # 登記所備付地図ビューア（client page・DataStore 非依存）
  api/                        # Route Handlers（parcel-towns / parcels / projects… / reset）
  tile/[z]/[x]/[y]/route.ts   # ZENRIN タイル中継
lib/
  db/{client,schema,init,seed-address,seed-parcels,sample,ids}.ts  # PostgreSQL データレイヤー
  queries/{projects,lands,buildings,visits,parcels,owners,serialize,helpers}.ts  # 読み書き + 導出ロジック
  geo.ts                      # parcelRing / polygonAreaTsubo / convexHull
...
components/
  ...
  project/{ProjectListClient,ProjectDetailClient,ProjectEditClient,LandDetailPanel,
           BuildingSection,LandAddDialog,ProjectCreateDialog,VisitAddForm,SummaryBar,StatusBar}.tsx
  report/ReportClient.tsx
...
> **同時ログイン数は 1**。`globalThis` セッションキャッシュはインスタンス単位なので、多インスタンス時は
> `10120004 同時ログイン数エラー` が起きうる。社内デモ用途では低トラフィックのため実害は小さいが、
> 必要なら単一インスタンス運用や外部セッションストア・ポーリング機能（`/api/auth/polling`）の導入を検討する。
...
- 土地は **公図風ビュー（左）+ 選択中土地パネル（右）** で CRUD（土地一覧テーブルは持たない）。
  パネルで地権者/坪数/備考/ステータスを編集、「筆を変更」で別の筆へ付け替え。
  パネルの「建物」セクション（`BuildingSection`）で建物の追加・編集・削除も行う
  （区分所有の建物は専有部分ごとに区分所有者・敷地権割合を編集）
- 「＋ 土地を追加」は **ワイドモーダル**（町名プルダウン + 大きな公図ビュー）。候補筆をクリックで
  その場追加（連続追加可・追加済みは候補から除外）

**案件詳細** / **業務報告** / **登記所備付地図ビューア** は「案件概要」記載のとおり。
...
Project { id, name, description, createdAt, updatedAt, polygon[[lat,lng]],
  ...
  └ Land { id, parcelId, aza, chiban, owners[{name, share}], description, areaTsubo, status,
           createdAt, updatedAt, polygon[[lat,lng]], buildings[], visits[] }
      ├ Building { id, name, houseNumber, structure, floorAreaTsubo, ownershipType,
      │            owners[{name, share}], units[], description, createdAt, updatedAt }
      │   └ BuildingUnit { id, unitNumber, owners[{name, share}], siteShare, description }
...
- 建物（`Building`）は土地に 1:N でぶら下がる**棟単位**のレコード。所在は土地（筆）が持つため
  建物自体はポリゴンを持たない
- `ownershipType` で地権者（所有者）の持ち方が変わる。**混在は構造上ありえない**:
  - `sole`（一棟所有 = 単独・共有）: 所有者は建物直下の `owners`（`building_owners` テーブル）。
    `units` は常に空
  - `kubun`（区分所有 = 分譲マンション等）: 所有者は専有部分 `units[]`（`building_units` ×
    `building_unit_owners`）ごとに持つ。建物直下の `owners` は常に空
- 所有形態を変更する PATCH は、旧形態側の所有者情報（sole→kubun なら owners、kubun→sole なら
  units）をサーバ側で破棄して不変条件を保つ（API 層は矛盾する owners/units の同時指定を 400 で拒否）
- 専有部分の `siteShare` は**敷地権割合**（土地に対する共有持分の `'分子/分母'` 文字列、未把握は `''`）。
  敷地権付き区分所有の土地は、地権者 = 区分所有者全員（持分 = 敷地権割合）として `Land.owners` に
  持たせる運用（サンプルの「上野パークハイツ」参照）。土地と建物の owners の自動同期は行わない
- 建物の所有者・区分所有者は土地の地権者と同じ `{name, share}` 形式で、名前文字列で紐づける
  （人物マスタへの正規化は現状しない。同一人物の土地・建物横断の名寄せは表示上の一致で読む）
...
| `lands` | id (`id_xxxx`), project_id (FK), parcel_id (FK), description, area_tsubo, status, created_at, updated_at | `UNIQUE (project_id, parcel_id)`。地権者は `land_owners` に正規化 |
| `buildings` | id (`id_xxxx`), land_id (FK), name, house_number, structure, floor_area_tsubo, ownership_type, description, created_at, updated_at | 建物（棟単位）。`ownership_type` は `CHECK IN ('sole','kubun')` |
| `building_owners` | id (IDENTITY PK), building_id (FK), name, share_num, share_den | 一棟所有（sole）の所有者。`land_owners` と同形 |
| `building_units` | id (IDENTITY PK), building_id (FK), unit_number, site_share_num, site_share_den, description | 専有部分（kubun のみ）。`UNIQUE (building_id, unit_number)`。敷地権割合は分子・分母の整数2列 |
| `building_unit_owners` | id (IDENTITY PK), unit_id (FK), name, share_num, share_den | 専有部分の区分所有者。`land_owners` と同形 |
...
- 案件・土地・建物の削除はカスケード。numeric（容積率・坪数）は postgres.js では文字列で返るため `serialize.ts` で数値化する
...
- できること: 案件・土地・建物（専有部分含む）のCRUD、ステータス変更、土地パネルからの訪問記録追加、サンプルデータへのリセット
...
- **筆マスタは町名単位の遅延取得**: 約 5.1 万筆を一括では読まない。`useParcelTowns()` /
  ...
  土地の領域・坪数・町名・地番はサーバが parcels から導出して返すため、クライアントは原則
