...
# ZENRIN API 認証情報・DATABASE_URL を .env（または .env.local）に設定（.env.example を参照）
# Next.js が自動で読み込む
...
一方 ZENRIN Web API はタイル取得に `x-api-key` + `Authorization: Bearer <token>` が必須。
そこで Route Handler `app/tile/[z]/[x]/[y]/route.ts` が橋渡しする:

- `lib/zenrin.ts` が `client_credentials` でアクセストークンを取得し、`expires_in` 内
  キャッシュ（`globalThis` 保持）して自動更新する（`oauth`/`ip`/`referer` の 3 方式に対応）
- Leaflet 側 URL は旧来どおり `/tile/{z}/{x}/{y}.png`。`[y]` は `25800.png` のように拡張子付きで
  届くのでハンドラ側で `.png` を剥がす（rewrites の `:param` は query へ展開されないため動的ルートにした）
...
lib/
  ...
  zenrin.ts                   # OAuth トークンキャッシュ + タイル取得
...
`DATABASE_URL` / `ZENRIN_AUTH_TYPE` / `ZENRIN_CLIENT_ID` / `ZENRIN_CLIENT_SECRET` /
`ZENRIN_API_KEY` / `ZENRIN_TOKEN_URL` / `ZENRIN_DOMAIN` / `ZENRIN_LAYER` / `ZENRIN_STYLE`
...
- 案件・土地の削除はカスケード。numeric（容積率・坪数）は postgres.js では文字列で返るため `serialize.ts` で数値化する
...
- **DB クライアント・OAuth トークン・init Promise は `globalThis` キャッシュ**（dev のホットリロードで
  多重接続/多重初期化しないように）。Shift-JIS CSV は `iconv-lite` で復号、筆マスタ COPY は
